...
const Search = ({ navigation }) => {
  ...
  const searchRecipes = async () => {
    const response = await fetch(
      `https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&query=${searchValue}&number=${recipesNumber}`
    )
  ...
  return (
    <Container>
      <RowContainer color="#eff7e1">
        ...
        />
        <PressableIcon onPress={searchRecipes}>
          <Icon source={searchImg} size="30" />
        </PressableIcon>
      </RowContainer>
      <Boxes array={searchCategories} />
    </Container>
  )
}
