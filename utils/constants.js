...
const SEARCH_URL = `${API_URL}/complexSearch?apiKey=${API_KEY}&instructionsRequired=true&number=${RECIPES_NUMBER}`

export const QUERY_SEARCH = (query) => `${SEARCH_URL}&query=${query}`

//step size 15 min
export const TIME_SEARCH = (time) =>
  `${SEARCH_URL}&maxReadyTime=${time}&minReadyTime=${time - 15}`

export const DIET_SEARCH = (diet) => `${SEARCH_URL}&diet=${diet}`

// step size 250 calories
export const CALORIES_SEARCH = (calories) =>
  `${SEARCH_URL}&maxCalories=${calories}&minCalories=${calories - 250}`

export const CATEGORY_SEARCH = (category) => `${SEARCH_URL}&type=${category}`
...
export const INGREDIENTS_SEARCH = (ingredients) =>
  `${API_URL}/findByIngredients?apiKey=${API_KEY}&number=${RECIPES_NUMBER}&ranking=2&ingredients=${ingredients.join(
    ','
  )}`
...
export const CLOUDINARY_PRESET = 'recipes_preset'
