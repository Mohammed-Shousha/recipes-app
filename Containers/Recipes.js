  ...
  return (
    <>
      {favourite && (!favRecipes || favRecipes.length == 0) ? (
        <Container center>
          <Icon source={emptyDish} size="100" />
          <Title>
            You have no favourite recipes, search and you will find many
            tasteful recipes!
          </Title>
        </Container>
          ...
          <ScrollView>
            {favRecipes.map((recipe) => (
              <RowContainer
                ...
                flexStart
                onPress={() =>
                  navigation.navigate('FavRecipe', { id: recipe.id })
                }
              >
              ...
              <RowContainer
                ...
                onPress={() =>
                  route.name === 'Recipes'
                    ...
                    : navigation.navigate('Search Recipe', { id: recipe.id })
                }
              >
                <RecipeMiniImage source={{ uri: recipe.image }} />
                <Text style={{ width: 240 }}>{recipe.title}</Text>
