...
const Boxes = ({
  ...
  const searchRecipesByTime = async (name) => {
    ...
    const response = await fetch(
      `https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&number=${recipesNumber}&maxReadyTime=${time}&minReadyTime=${
        time - 15
  ...
  const searchRecipesByDiet = async (diet) => {
    const response = await fetch(
      `https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&number=${recipesNumber}&diet=${diet}&instructionsRequired=true`
    )
    const { results } = await response.json()
    setRecipes(results)
    navigation.navigate('Search Recipes')
  ...
  const searchRecipesByCalories = async (name) => {
    ...
    const response = await fetch(
      `https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&number=${recipesNumber}&maxCalories=${calories}&minCalories=${
        calories - 250
      }&instructionsRequired=true`
    )
    const { results } = await response.json()
  ...
  const redirect = async (name) => {
    const response = await fetch(
      `https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&number=${recipesNumber}&type=${name}&instructionsRequired=true`
    )
    const { results } = await response.json()
    setRecipes(results)
        ...
        <BoxesContainer key={i}>
          {Two.map((C, i) => (
            <Box
              ...
              onPress={
                time
                  ? () => searchRecipesByTime(C.name)
                  : diet
                  ? () => searchRecipesByDiet(C.name)
                  : calories
                  ? () => searchRecipesByCalories(C.name)
                  : () => redirect(C.name)
              }
